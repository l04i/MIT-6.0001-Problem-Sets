    ...
    for letter in word :
        if letter in hand:
            if new_hand[letter] > 1 :
                new_hand[letter] = hand.get(letter,0) - 1 
            else:
                del(new_hand[letter])
    return new_hand
#
...
def is_valid_word(word, hand, word_list):
    ...
    hand_copy = hand.copy()
    word = word.lower()
    letters_count_in_hand = []
    for letter in word:
        if letter in hand_copy:
            if hand_copy[letter] > 1 :
                letters_count_in_hand.append(hand_copy.get(letter,0))   
                hand_copy[letter] = hand_copy.get(letter,0) -1
            else:
                del(hand_copy[letter])
        else:
            letters_count_in_hand.append(0)     
    matches = []    
    if '*' in word:
        index = word.find('*')
        for letter in VOWELS:
            ...
            if word in word_list:
                matches.append(word)
    all_letters_in_hand = not (0 in letters_count_in_hand)
    word_exists = word in word_list
    
    return all_letters_in_hand and (word_exists or len(matches) != 0)
#
    ...
    * The hand finishes when there are no more unused letters.
    ...
    while hand_length != 0:
        ...
        if is_valid_word(word, hand, word_list):
            ...
            print('"fix" earned',word_score,'points. Total:',hand_score,'points')
        else:
    ...
    if hand_length == 0:
        print('Ran out of letters. Total score:', hand_score ,'points')
    else:
...
def substitute_hand(hand, letter):
    ...
    for x in letters_in_hand:
        if x != '*':
            allowed_letters.remove(x)
    new_letter = random.choice(allowed_letters)
    del(new_hand[letter])
