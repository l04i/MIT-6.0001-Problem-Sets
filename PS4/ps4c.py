    ...
    def apply_transpose(self, transpose_dict):
        ...
        for letter in self.message_text:
            if letter in transpose_dict.keys():
                encrypted_message_text.append(transpose_dict[letter])
            else:
                encrypted_message_text.append(letter)
...
class EncryptedSubMessage(SubMessage):
    ...
    def decrypt_message(self):
        ...
        for permutation in get_permutations(VOWELS_LOWER):
            ...
            decrypted_message_words = decrypted_message.split()
            for word in decrypted_message_words:
                if is_word(wordlist,word):
                    result[decrypted_message] = result.get(decrypted_message,0) + 1 
